...
var dogeauthMiddleware = require('../lib/middleware/dogeauth');
...
app.get('/user', dogeauthMiddleware, function(req, res) {
  if(!req.sin || !users[req.sin]) return res.send(401, {error: 'Unauthorized'});
  res.send(200, users[req.sin]);
});

app.post('/pizzas', dogeauthMiddleware, function(req, res) {
  if(!req.sin || !users[req.sin]) return res.send(401, {error: 'Unauthorized'});
  var pizza = req.body;
